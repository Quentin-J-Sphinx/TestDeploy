...
const usersController = {
    ...
    async login(req, res) {
        ...
        try {
            ...
            if (user && validPass) {
                ...
                const token = jwt.sign({ id: user.id, mail: user.mail, role: user.role, firstname: user.firstname, lastname: user.lastname, address: user.address, number: user.number, confirmed: user.isConfirmed }, 'your-secret-key', { expiresIn: '1h' });
    ...
    async checkEmail(req, res) {
        ...
        try {
            ...
            console.error(`Erreur à la vérification de l'email`, error);
            res.status(500).json({ message: `Erreur lors de la vérification de l'émail` });
        }
    }
};
